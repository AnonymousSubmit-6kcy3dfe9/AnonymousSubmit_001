...
This produces 13 initial public problem records. The excluded items may be
kept in a private audit note, but must not appear in the public question index.
The 24-count taxonomy table is a target inventory for the broader atlas, not a
claim that the first release contains 24 records.
...
the problem, without publishing the result itself. A `this_work` event has no
citation label and may not contain author names, URLs, DOI/ePrint identifiers,
email addresses, or answer-bearing artifact references. The release checker
enforces the machine-detectable parts of this boundary.
...
The present release populates only symmetric cryptography. Within that area,
the three retained research domains are foundations and structural questions,
design of cryptographic primitives, and cryptanalysis.
...
evidence facet, not a cryptographic category.
...
block ciphers, S-boxes and nonlinear components, differential cryptanalysis,
and an `others` fallback. The next taxonomy revision should add dedicated
leaves for implementation/XOR complexity, GFN/diffusion, boomerang analysis,
and algebraic immunity once enough public records justify them. Existing IDs
...
1. Freeze the taxonomy and schema; create the 13 audited public records with
